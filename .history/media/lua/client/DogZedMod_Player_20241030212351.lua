...
function DogZedMod.wearDogZed(pl, int)
	...
	if int == 2 then
		item = InventoryItemFactory.CreateItem(DogZedMod.skin0)
	end
...
function DogZedMod.AdminDogZed(pl)
	if DogZedMod.isDogPl(pl) then
--[[ 		local int = DogZedMod.getDayTimeInt()
		if DogZedMod.getAdminSkinNum(pl) ~= int then
			DogZedMod.wearDogZed(pl, int)
		end ]]

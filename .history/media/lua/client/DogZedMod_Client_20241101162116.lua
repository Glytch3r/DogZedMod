...
function DogZedMod.coreFunc(zed)
    ...
    if DogZedMod.isDogZed(zed) then
        ...
        if zed:getVariableString('isDogZed') or init then
...
Events.OnZombieUpdate.Remove(DogZedMod.coreFunc)
Events.OnZombieUpdate.Add(DogZedMod.coreFunc)
...
Commands.DogZedMod.KnockDown = function(args)
    local source = getPlayer();
    local player = getPlayerByOnlineID(args.id)
    local zedID = args.zedID
    if type(zedID) == 'string' then zedID = tonumber(zedID) end
...
Commands.DogZedMod.isDogPl = function(args)
    local source = getPlayer();
    local player = getPlayerByOnlineID(args.id)
    if source ~= player then
        if args.isDogPl then
            DogZedMod.wearDogZed(player, int)
			if not player:isHideWeaponModel() then player:setHideWeaponModel(true) end
            if player:getVariableBoolean('isDogPl') == false then
                player:setVariable('isDogPl', true);
            end
        else
            DogZedMod.clearDogZedSkin(player)
			if player:isHideWeaponModel() then player:setHideWeaponModel(false) end
            if player:getVariableBoolean('isDogPl')  then
                player:setVariable('isDogPl', false);
            end
        end
    end
end

Commands.DogZedMod.Injury = function(args)
    local source = getPlayer();
   --local player = getPlayerByOnlineID(args.id)
	local targ = getPlayerByOnlineID(args.targID)
    if source ~= targ then
		DogZedMod.doInjury(targ)
    end
end

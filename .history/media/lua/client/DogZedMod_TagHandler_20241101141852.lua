...
local function doRadiateArea(obj, activate)
    local decay = 1
    if activate then
    ...
    local col = ColorInfo.new(0, 1, ZombRand(0.005, 0.3), 0.001);
...
function DogZedMod.setRadiateSq(sq)
    if sq then
            ...
            if obj and instanceof(obj, "IsoObject") then
                ...
                if flr and flr ~= obj then
                    if DogZedMod.doRoll(35) then
                        doRadiateArea(obj, true)
                        timer:Simple(3, function()
                            doRadiateArea(obj, false);
                        end)
                    end
                end
            end
...
function DogZedMod.setRadiateArea(targ)
    local count = 0
    local rad = 3
    local cell = getCell()
...
function DogZedMod.getRandSq(centerPoint)

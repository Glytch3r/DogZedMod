...
function DogZedMod.hitZed(zed, pl, part, wpn)
	...
	if DogZedMod.isDogPl(pl) then
		...
		if  DogZedMod.isDogZed(zed) then
			if DogZedMod.isRadiatedDog then
				DogZedMod.addHL(zed)
			end
...
function DogZedMod.InjuryRoll(pl)
	...
	if DogZedMod.isRadiatedDog(zed) then
		DogZedMod.setRadiateSq(DogZedMod.getRandSq(zed, ZombRand(1, 4)))
	end
...
function DogZedMod.hit(zed, pl, wpn, HP)
	...
	if DogZedMod.isDogPl(pl)  then
		pl:setBumpType("Dog_HitReact");
		pl:setVariable("BumpFall", true);
		pl:setVariable("BumpFallType", "Dog_HitReact");
	end
	...
	if DogZedMod.isDogZed(zed) and pl == getPlayer() then
		...
		if DogZedMod.isRadiatedDog(zed) then  DogZedMod.setRadiateSq(pl) end
	end

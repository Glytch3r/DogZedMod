...
function DogZedMod.Context(player, context, worldobjects)
	...
	if sq then
		...
		if (getCore():getDebug() or isAdmin()) then
			...
			Main.iconTexture = getTexture("media/ui/DogZed_ico.png")
			...
			if not DogZedMod.isDogPl(pl) then
				local ss = opt:addOption(getText('ContextMenu_DogZedMod_WearSkin'), worldobjects, function()
					DogZedMod.wearDogZed(pl, 1)
				end);
